module github.com/findy-network/findy-agent-cli

go 1.15

require (
	github.com/findy-network/findy-agent v0.0.0-20201114131912-c55ec5319725
	github.com/findy-network/findy-agent-api v0.0.0-20201114124900-60e2aa27124d
	github.com/findy-network/findy-grpc v0.0.0-20201113091436-e47cd902d2c7 // indirect
	github.com/findy-network/findy-wrapper-go v0.0.0-20201026120712-dc9ac6619595
	github.com/golang/glog v0.0.0-20160126235308-23def4e6c14b
	github.com/google/uuid v1.1.2 // indirect
	github.com/lainio/err2 v0.5.0
	github.com/sideshow/apns2 v0.20.0 // indirect
	github.com/spf13/cobra v1.1.1
	github.com/spf13/pflag v1.0.5
	github.com/spf13/viper v1.7.1
	google.golang.org/grpc v1.33.1
)
